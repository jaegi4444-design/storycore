...
import { initializeSampleDataIfEmpty } from '../data/sampleData';
...
export function App() {
  ...
  const { works, addWork, updateWork, deleteWork, getWorkById } = useWorks();
  const { addCharacter, updateCharacter, deleteCharacter, getCharactersByWorkId } =
    useCharacters();
  const { addEpisode, updateEpisode, deleteEpisode, getEpisodesByWorkId } = useEpisodes();
  const {
    addWorldSetting,
    updateWorldSetting,
    deleteWorldSetting,
  ...
  const [selectedWorkId, setSelectedWorkId] = useLocalStorageString(
    STORAGE_KEYS.selectedWorkId,
    null,
  );
