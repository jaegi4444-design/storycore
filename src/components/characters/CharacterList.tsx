...
type CharacterListProps = {
  ...
  onAdd: (data: CharacterInput) => void;
  onUpdate: (id: string, data: CharacterInput) => void;
    ...
    <div className="space-y-6">
      ...
      {characters.length === 0 ? (
        <EmptyState
          ...
          action={
            <Button onClick={() => setFormOpen(true)}>캐릭터 추가</Button>
          }
        />
      ) : (
      ...
      <CharacterForm
        ...
        onSubmit={(data) => {
          if (editingCharacter) {
            onUpdate(editingCharacter.id, data);
          } else {
            onAdd(data);
          }
          setFormOpen(false);
