...
import { buildFilterOptions, hasRequiredWorkCategories } from '../../types/work';
...
export function CharacterList({
  ...
  const jobFilterOptions = useMemo(
    () =>
      work
        ? buildFilterOptions(
            work.jobs,
            characters.map((c) => c.job),
            '전체 직업',
          )
        : [{ value: ALL_VALUE, label: '전체 직업' }],
    [work, characters],
  );
  const affiliationFilterOptions = useMemo(
    () =>
      work
        ? buildFilterOptions(
            work.affiliations,
            characters.map((c) => c.affiliation),
            '전체 소속',
          )
  ...
  const handleAddClick = () => {
    if (!categoriesReady) {
      alert('작품에 등급·직업·소속을 먼저 등록해주세요. (작품 관리)');
      return;
    }
    ...
    <div className="space-y-6">
      ...
      {!categoriesReady && (
        <div className="rounded-lg border border-amber-500/30 bg-amber-500/10 px-4 py-3 text-sm text-amber-200">
          등급·직업·소속 카테고리를 작품 관리에서 등록하면 캐릭터 추가 시 셀렉트로 선택할 수 있습니다.
        </div>
      )}
      ...
      {characters.length > 0 && (
        <div className="rounded-xl border border-surface-border bg-surface-raised p-4">
          ...
          {categoriesReady ? (
            <>
              <div className="mb-2 hidden grid-cols-3 gap-3 px-1 text-xs font-medium sm:grid">
                <span className="text-violet-300">등급</span>
                <span className="text-sky-300">직업</span>
                <span className="text-emerald-300">소속</span>
              </div>
      ...
      {characters.length === 0 ? (
        ...
        <div className="grid gap-6 lg:grid-cols-5">
          <div className="space-y-3 lg:col-span-2">
            <div className="hidden grid-cols-3 gap-2 px-1 text-[10px] font-medium uppercase tracking-wide text-gray-500 sm:grid">
              <span className="text-violet-300">등급</span>
              <span className="text-sky-300">직업</span>
              <span className="text-emerald-300">소속</span>
            </div>
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-1">
              {filteredCharacters.map((character) => (
                <CharacterCard
                  ...
                  highlightRank={filterRank}
                  highlightJob={filterJob}
                  highlightAffiliation={filterAffiliation}
