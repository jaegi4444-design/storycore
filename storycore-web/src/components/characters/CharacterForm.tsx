...
import { hasRequiredWorkCategories, toCharacterSelectOptions } from '../../types/work';
...
export function CharacterForm({
  ...
  const rankOptions = useMemo(
    () => toCharacterSelectOptions(work.ranks, form.rank),
    [work.ranks, form.rank],
  );
  const jobOptions = useMemo(
    () => toCharacterSelectOptions(work.jobs, form.job),
    [work.jobs, form.job],
  );
  const affiliationOptions = useMemo(
    () => toCharacterSelectOptions(work.affiliations, form.affiliation),
    [work.affiliations, form.affiliation],
  );
  ...
  const handleSubmit = async () => {
    ...
    if (!categoriesReady) {
      setError('작품에 등급·직업·소속을 먼저 등록해주세요. (작품 관리)');
      return;
    }
  ...
  return (
    ...
    >
      ...
      <div className="mt-4 rounded-lg border border-surface-border bg-surface-overlay/30 p-4">
        ...
        <p className="mb-4 text-xs text-gray-500">작품에 등록된 카테고리에서 선택합니다.</p>
